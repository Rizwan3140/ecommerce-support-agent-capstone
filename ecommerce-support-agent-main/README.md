...
- Integrates the supplied sample dataset into the RAG corpus:
  - `data/policies/capstone_sample_faq.md`
  - `data/policies/capstone_sample_policies.md`
  - `data/policies/capstone_product_catalog.md`
- Adds mock backend APIs in `tools/mock_apis.py`:
  - `get_order_status(order_id)`
  - `create_return_request(order_id, reason)`
  - `get_refund_policy()`
  - `search_products(query)`
- Exposes mock API endpoints:
  - `GET /tools/order-status/{order_id}`
  - `POST /tools/returns`
  - `GET /tools/refund-policy`
  - `GET /tools/products?q=monitor`
- Shows tool calls in the Streamlit UI and final JSON response.
- Adds deliverable files:
  - `docs/technical_report.md`
  - `docs/presentation_deck.md`
  - `evaluation/capstone_queries.csv`
  - `evaluation/sample_conversation_logs.md`

Suggested demo prompts:

- `Where is my order #1002?`
- `Where is my order?`
- `I want to return order #1001 because it is unused and still in the box.`
